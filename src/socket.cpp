...
int Socket::Bind() {
    if(port == 0) {
        std::cout << "ERROR on binding: Port not defined." << std::endl;
        return -1;
    }
...
int Socket::Bind(int port) {
    ...
    if(port < 1) {
        std::cout << "ERROR on binding: Invalid port." << std::endl;
        return -1;
    }
...
int Socket::Read(t_byte * buffer, size_t size) {
    int r;
...
int Socket::Send(t_byte * buffer, size_t size) {
    int r;
...
int Socket::Reply(t_byte * buffer, size_t size) {
    int r;
...
int Socket::Receive(t_byte * buffer, size_t size) {
    int r;
